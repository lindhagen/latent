...
for details, we refer to \cite{lindhagen_ostlund} and the
references therein.
...
The methods of the package rests on three statistical models, specified by the
...
The three models to be specified are the following:
\begin{enumerate}
...
other models is whatever outcome $Y$ is recorded in the study. It can for
example be numerical, dichotmous or a time to event outcome.
All models can use any number of covariates as predictors of subgroup or outcome
to increase the precision. However, the model for treatable patients will
always contain randomization, which the model for untreatable patients
will not.
...
misspecified models can be found in \cite{lindhagen_ostlund}.
...
\cite{lindhagen_ostlund}. Thus, we shall generate data on $n = 3000$ patients,
with a subgroup prevalence of 25\%.
...
\noindent
Next, we create the ''true'' data, containing the latent subgroup.
\begin{routput}[H]
<<"basic-true-data">>=
df_true <- data.frame(
    g = rbinom(n, size = 1, prob = 0.25), # Latent subgroup (still observed).
    rand = rbinom(n = n, size = 1, prob = 0.5)) # Randomization.
# Outcome.
...
database that we would actually observe in the study:
\begin{routput}[H]
...
We now set up the three statistical models required by the
...
\noindent
Note that the outcome of this model is the subgroup $G$, coded as the variable
\texttt{g}. We have not added
...
model0 <- latent_linear(
    formula = (y ~ 1),
    coef_names = "alpha0")
model1 <- latent_linear(
    formula = (y ~ rand),
...
for technical details, see \cite{lindhagen_ostlund}.
...
\begin{routput}[H]
<<ord-outcome-models>>=
model0 <- latent_ordinal(
    formula = (y ~ 1),
    K = 4,
    coef_names = NULL)
model1 = latent_ordinal(
    formula = (y ~ rand),
    K = 4,
    coef_names = "psi")
@
...
and Parmar. Again, we refer to \cite{lindhagen_ostlund} for details.
...
\begin{routput}[H]
<<surv-outcome-models>>=
model0 <- latent_flexsurv_ph(
    formula = (y ~ 1),
    knots = knots,
    coef_names = NULL)
model1 <- latent_flexsurv_ph(
    formula = (y ~ rand),
    knots = knots,
    coef_names = "psi")
@
...
to the models in order to increase the power. This idea is well-known for the
...
patients, and hence take a step in the direction of fully identifying the
...
\bibitem{lindhagen_ostlund} L.\ Lindhagen and O.\ \"Ostlund,
