...
#' }
#' @details For details on the usage of this function, see the package vignette.
...
latent_main <- function(data,
  ...
  setup.likelihood.data <- function(data, theta) {
    ret <- data %>%
      mutate(
        ...
        delta0 = ifelse(!is.na(g) & (g == 1), 0, 1),
        delta1 = ifelse(!is.na(g) & (g == 0), 0, 1),
        ## Model likelihoods.
        # Likelihood of g = 0, 1.
        MS0 = latent_likelihood(modelS, theta = theta$thetaS,
          data = mutate(data, g = 0)),
        MS1 = 1 - MS0,
        # Outcome models.
  ...
  # For thetaS, we fit the model on {rand = 1}, where g is observed.
  # For theta0 and theta1, fit weighted models, where the weights for unobserved
  # g are based on the predicted probabilities from the subgroup model.
  thetaS_start <- latent_fit(modelS, data = data, weights = rep(1, nrow(data)))
  # Compute predicted subgroup probabilities.
  dd_start <- data %>%
    mutate(g_obs = g) %>%
    mutate(g = 1) # Want the probability of g = 1.
  dd_start %<>% mutate(
    pg = latent_likelihood(modelS, theta = thetaS_start, data = dd_start),
    wt = ifelse(is.na(g_obs), pg, g_obs), # Weight = g when g observed.
    # Keep the weights away from 0 and 1 (prevents negative hazards in
  ...
  if (do_em) {
    ...
    for (em_ix in 2:max_em_iter) {
      ...
      dd <- data %>%
        mutate(
          # Prior probabilities.
          pi0 = ifelse(is.na(g), MS0, 1 - g),
          pi1 = ifelse(is.na(g), MS1, g),
      ...
      # Subgroup model. Augment the data so that each patient contributes two
      # rows, with (g, weight) = (0, 1 - w) and (1, w), respectively.
      ddS0 <- dd %>% mutate(g = 0, wt = 1 - w)
      ddS1 <- dd %>% mutate(g = 1, wt = w)
      ddS <- rbind(ddS0, ddS1)
    ...
    loglik_gradS0_list <- latent_loglik_gradient_list(
      modelS, theta = theta_em$thetaS, data = mutate(data, g = 0))
    loglik_gradS1_list <- latent_loglik_gradient_list(
      modelS, theta = theta_em$thetaS, data = mutate(data, g = 1))
